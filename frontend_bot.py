...
import sqlite3  # Добавьте этот импорт если его нет
...
async def cancel_task_menu(callback: types.CallbackQuery):
    ...
    if not user_tasks:
        await callback.answer("У вас нет задач для отмены", show_alert=True)
        return
...
async def cancel_task_execute(callback: types.CallbackQuery):
    ...
    cursor.execute(
        "SELECT id, status FROM parsing_tasks WHERE id = ? AND user_id = ?",
        (task_id, callback.from_user.id)
    )
...
async def process_limit(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        result_text = (
            ...
            "Я пришлю уведомление, когда результат будет готов!\n\n"
            "<i>Используйте /tasks для отслеживания статуса.</i>"
        )
